...
- `adapters`: parse a CLI's local data into conversations, turns, model calls, tool
  ...
  Claude Code, Continue CLI, Crush, Gemini CLI, GitHub Copilot CLI, Goose, Kilo Code,
  OpenCode, Pi, and Qwen Code expose the core dimensions available in their local
  stores.
