...
10,000 discovered candidates per collection; 64 MiB per monolithic JSON file; 256 MiB
per JSONL file and 8 MiB per line; and, for SQLite stores, 512 MiB per database,
250,000 selected rows, 8 MiB per structured field, and 256 MiB across structured
fields. Exceeding a limit aborts collection with a generic code rather than silently
