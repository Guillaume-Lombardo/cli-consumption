...
@cache
def _dashboard_calculations_script() -> str:
    """Load the pure calculations that are inlined into every dashboard."""
    return (
        files("cli_consumption")
        .joinpath("dashboard_calculations.js")
...
const {{activeMs,cohortComparison,compareMetric,conversationByKey:convByKey,day,inRange,maxConcurrent,metrics,percentile,rangeFor,ratio,selectSlice,semanticTokenCalls,total,validDate}}=calculations;
...
function selected(){{return{{provider:$('provider').value,machine:$('machine').value,project:$('project').value,model:$('model').value,range:rangeFor($('period').value,{{from:$('from').value,to:$('to').value}})}};}}
function delta(current,previous,preference='neutral'){{const comparison=compareMetric(current,previous,preference);if(!comparison)return'';return`<div class="delta ${{comparison.style}}">${{comparison.change>=0?'+':''}}${{comparison.change.toFixed(1)}}% vs previous period</div>`;}}
...
function renderCohorts(s){{const entries=cohortComparison(s,$('cohortDimension').value);$('cohorts').innerHTML=entries.length?`<table><thead><tr><th>Cohort</th><th>Closed turns</th><th>Median duration</th><th>Median tokens</th><th>Tools / turn</th><th>Context p95</th><th>Abort rate</th></tr></thead><tbody>${{entries.map(r=>`<tr><td>${{escapeHtml(r.label)}}</td><td>${{fmt(r.turns)}}</td><td>${{formatDuration(r.durationP50)}}</td><td>${{short(r.tokensP50)}}</td><td>${{r.toolsPerTurn.toFixed(1)}}</td><td>${{pct(r.pressureP95)}}</td><td>${{pct(r.abortRate)}}</td></tr>`).join('')}}</tbody></table>`:'<div class="empty">No cohort has enough data for this selection.</div>';}}
