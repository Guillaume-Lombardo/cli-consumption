...
(() => {
  ...
  var DASHBOARD_DATASET_VERSION = 1;
  var DATASET_SECTIONS = [
    "conversations",
    "turns",
    "modelCalls",
    "toolCalls",
  ...
  function assertDashboardDatasetV1(value) {
    if (typeof value !== "object" || value === null) {
      throw new TypeError("invalid_dashboard_dataset");
    }
    const candidate = value;
    const meta = candidate.meta;
    if (candidate.contractVersion !== DASHBOARD_DATASET_VERSION || typeof meta !== "object" || meta === null || typeof meta.shareSafe !== "boolean" || DATASET_SECTIONS.some((section) => !Array.isArray(candidate[section]))) {
      throw new TypeError("invalid_dashboard_dataset");
    }
  }

  // ../analytics/src/index.ts
  function createDashboardCalculations(input) {
    ...
    const settingByTurn = Object.fromEntries(
      data.turnSettings.filter((setting) => setting.turnKey !== null).map((setting) => [setting.turnKey, setting])
    );
    ...
    const total = (rows, key) => rows.reduce(
      (sum, row) => sum + (Number(row[key]) || 0),
      0
    );
    const ratio = (numerator, denominator) => denominator ? 100 * numerator / denominator : 0;
    function percentile(values, probability) {
      ...
      if (lowerValue === void 0 || upperValue === void 0) return null;
    ...
    function rangeFor(period, custom = {}) {
      ...
      const exportStart = validDate(data.meta.exportWindow?.since) ? new Date(data.meta.exportWindow.since) : null;
      const exclusiveEnd = validDate(data.meta.exportWindow?.until) ? new Date(data.meta.exportWindow.until) : null;
      const exportEnd = exclusiveEnd ? new Date(exclusiveEnd.getTime() - 1) : null;
      const latest = dates.reduce(
        (maximum2, value) => Math.max(maximum2, value),
        -Infinity
      );
      ...
      let end = maximum;
      if (period === "custom") {
        start = custom.from ? /* @__PURE__ */ new Date(`${custom.from}T00:00:00Z`) : exportStart;
        end = custom.to ? /* @__PURE__ */ new Date(`${custom.to}T23:59:59.999Z`) : maximum;
      } else if (period !== "all") {
    ...
    function inEpochRange(value, range) {
      if (value === null || value === void 0) return !range?.start;
      const time = Number(value);
      return (!range?.start || time >= range.start.getTime()) && (!range || time <= range.end.getTime());
    }
    function conversationInRange(conversation, range) {
      ...
      const start = validDate(conversation.startedAt) ? Date.parse(conversation.startedAt) : -Infinity;
      const end = validDate(conversation.endedAt) ? Date.parse(conversation.endedAt) : Infinity;
      return (!range.start || end >= range.start.getTime()) && start <= range.end.getTime();
    }
    function selectSlice(filters, range = filters.range) {
      const base = data.conversations.filter(
        (conversation) => (!filters.provider || conversation.provider === filters.provider) && (!filters.machine || conversation.machine === filters.machine) && (!filters.project || conversation.project === filters.project) && (!filters.model || conversation.models.includes(filters.model))
      );
      const keys = new Set(base.map((conversation) => conversation.key));
      let calls = data.modelCalls.filter((call) => {
        if (!keys.has(call.conversationKey) || filters.model && call.model !== filters.model) {
          return false;
        }
        ...
        if (semantics === "conversation-aggregate" || semantics === "context-snapshot") {
          return conversation !== void 0 && conversationInRange(conversation, range);
        }
        return validDate(call.timestamp) && inRange(call.timestamp, range);
      });
      const modelTurns = filters.model ? new Set(calls.map((call) => call.turnKey).filter((key) => key !== null)) : null;
      const turns = data.turns.filter(
        (turn) => keys.has(turn.conversationKey) && inRange(turn.startedAt, range) && (!modelTurns || modelTurns.has(turn.key))
      );
      ...
      calls = calls.filter((call) => {
        ...
        return semantics === "conversation-aggregate" || semantics === "context-snapshot" || call.turnKey === null || allowedTurns.has(call.turnKey);
      });
      const tools = data.toolCalls.filter(
        (tool) => keys.has(tool.conversationKey) && inRange(tool.timestamp, range) && (!filters.model || tool.turnKey !== null && allowedTurns.has(tool.turnKey))
      );
      const work = data.workItems.filter(
        (item) => keys.has(item.conversationKey) && (!filters.model || item.turnKey !== null && allowedTurns.has(item.turnKey)) && inEpochRange(item.startedAtMs, range)
      );
      const contexts = data.contextSamples.filter(
        (sample) => keys.has(sample.conversationKey) && inRange(sample.timestamp, range) && (!filters.model || sample.turnKey !== null && allowedTurns.has(sample.turnKey))
      );
      const settings = data.turnSettings.filter(
        (setting) => keys.has(setting.conversationKey) && setting.turnKey !== null && allowedTurns.has(setting.turnKey)
      );
      const compactions = data.compactions.filter(
        (compaction) => keys.has(compaction.conversationKey) && inRange(compaction.timestamp, range) && (!filters.model || compaction.turnKey !== null && allowedTurns.has(compaction.turnKey))
      );
      const activeConversationKeys = /* @__PURE__ */ new Set([
      ...
      const conversations = base.filter(
        (conversation) => (conversationInRange(conversation, range) || activeConversationKeys.has(conversation.key)) && (!filters.model || calls.some((call) => call.conversationKey === conversation.key))
      );
      const activeKeys = new Set(conversations.map((conversation) => conversation.key));
      const subagents = data.subagents.filter(
        (subagent) => subagent.conversationKey !== null && activeKeys.has(subagent.conversationKey) && inEpochRange(subagent.createdAtMs, range)
      );
      return {
        conversations,
        turns,
    ...
    function semanticTokenCalls(slice) {
      const closedKeys = new Set(
        slice.turns.filter((turn) => turn.status === "completed" || turn.status === "aborted").map((turn) => turn.key)
      );
      return slice.calls.filter((call) => {
        ...
        return semantics === "additive" && (call.turnKey === null || closedKeys.has(call.turnKey));
      });
    }
    function metrics(slice) {
      ...
      const durations = closed.map((turn) => turn.durationMs).filter((value) => value !== null);
      ...
      const input2 = total(tokenCalls, "input_tokens");
      const cached = total(tokenCalls, "cached_input_tokens");
      const output = total(tokenCalls, "output_tokens");
      const active = activeMs(closed);
      const pressures = slice.contexts.map(
        (sample) => 100 * Number(sample.inputTokens) / Number(sample.contextWindowTokens)
      ).filter(Number.isFinite);
      const activeDays = new Set(
        slice.turns.map((turn) => day(turn.startedAt)).filter((value) => value !== "unknown")
      ).size;
      return {
        ...
        activeMs: active,
        throughput: active ? 36e5 * closed.length / active : 0,
    ...
    function compareMetric(current, previous, preference = "neutral") {
      if (previous === null || previous === void 0 || !Number.isFinite(previous) || previous === 0) {
        return null;
      }
      const change = 100 * (current - previous) / Math.abs(previous);
      const better = preference === "higher" ? change > 0 : preference === "lower" ? change < 0 : null;
      return {
        change,
    ...
    function cohortComparison(slice, dimension) {
      ...
      slice.turns.filter((turn) => turn.status === "completed" || turn.status === "aborted").forEach((turn) => {
      ...
      return Object.entries(rows).filter(([, row]) => !data.meta.shareSafe || row.turns.length >= 5).sort((left, right) => right[1].turns.length - left[1].turns.length).map(([label, row]) => ({
        label,
  ...
  // src/index.ts
  globalThis.createDashboardCalculations = createDashboardCalculations;
})();
